...
class EloRewardService {
    ...
    /**
     * Lấy lịch sử nhận thưởng milestone của user
     * @param {number} userId - ID người dùng
     * @param {number} seasonId - ID mùa (optional, null = tất cả)
     * @returns {Promise<Array>} Lịch sử claim
     */
    static async getClaimHistory(userId, seasonId = null) {
        try {
            let query = `
                SELECT 
                    c.claim_id,
                ...
                JOIN reward_seasons s ON c.season_id = s.season_id
                WHERE c.user_id = ?
            `;
    ...
    /**
     * Lấy danh sách tất cả các mùa
     * @returns {Promise<Array>} Danh sách seasons
     */
