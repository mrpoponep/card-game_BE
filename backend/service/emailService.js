...
/**
 * Gửi email với mã OTP để xác thực email khi đăng ký
 * @param {string} email - Email người nhận
 * @param {string} username - Tên đăng nhập
 * @param {string} otpCode - Mã OTP 6 số
 * @returns {Promise<boolean>}
 */
export async function sendEmailVerificationOTP(email, username, otpCode) {
  const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1>🎴 Card Game - Xác thực Email</h1>
        </div>
        <div class="content">
          <p>Xin chào <span class="username">${username}</span>,</p>
          <p>Cảm ơn bạn đã đăng ký tài khoản tại Card Game!</p>
          <p>Để hoàn tất việc liên kết email với tài khoản của bạn, vui lòng sử dụng mã xác thực sau:</p>
          ...
          <div class="warning">
            <p><strong>⚠️ Lưu ý quan trọng:</strong></p>
            <p>• Mã này chỉ có hiệu lực trong <strong>15 phút</strong></p>
            <p>• Email này giúp bạn khôi phục tài khoản khi quên mật khẩu</p>
            <p>• Không chia sẻ mã này với bất kỳ ai</p>
          </div>
  ...
  const mailOptions = {
    from: process.env.EMAIL_FROM || process.env.EMAIL_USER,
    to: email,
    subject: '🎴 Card Game - Xác thực Email đăng ký',
    html: htmlContent
  };
...
export default { sendPasswordResetOTP, sendEmailVerificationOTP };
